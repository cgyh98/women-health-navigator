...
function DocumentsScreen({ docs, setDocs }: { docs: Doc[]; setDocs: React.Dispatch<React.SetStateAction<Doc[]>> }) {
  ...
  function addMockDoc(fileList: FileList | null) {
  ...
  return (
    <div className="screen-pad">
      ...
      <div className="upload-zone" onClick={() => fileRef.current?.click()}>
        ...
        <input ref={fileRef} type="file" multiple hidden onChange={(e) => addMockDoc(e.target.files)} />
      </div>
      {docs.length === 0 ? (
...
function SummaryScreen({ userTurnCount }: { userTurnCount: number }) {
