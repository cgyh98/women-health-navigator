...
// Scripted, on-device-feeling demo agent. No network calls — this stands in for
// the real local model (Gemma/Ollama) so the UI/UX can be fully rehearsed offline.
// Swap `nextAgentMessage` / `buildSummary` for real backend calls once that's ready.
...
function nextAgentMessage(turnIndex) {
  return agentTurns[Math.min(turnIndex, agentTurns.length - 1)];
}

function shouldShowProviderNudge(userMessageCount) {
...
// Cumulative summary that fills in as the conversation progresses.
const summaryStages = [
  null,
  {
    summary:
      "You're describing symptoms that started recently. I'm gathering a bit more detail before putting together next steps.",
    next_steps: [],
    questions: [],
    red_flags: [],
  },
  {
    summary:
      "Based on what you've shared so far, this sounds like a manageable, common concern — I'll keep refining this as we talk.",
    next_steps: [
  ...
  {
    summary:
      "This sounds like a common concern that's worth mentioning at your next visit, but nothing you've described so far sounds urgent.",
    next_steps: [
      "Keep a simple log of when symptoms happen and how severe they feel.",
  ...
  {
    summary:
      "Here's where things stand: your symptoms sound consistent with a common, non-urgent issue, and nothing you've described points to an emergency. It's still worth a conversation with your provider so it's on record and properly ruled out.",
    next_steps: [
      "Keep a simple log of when symptoms happen and how severe they feel.",
      "Note anything that seems to trigger or worsen it.",
...
function buildSummary(userMessageCount) {
...
function PhoneFrame({ children, footer }) {
  return (
...
const slides = [
  {
    gradient: "linear-gradient(160deg, #8f7fe0 0%, #c9a8e0 45%, #f3d9c4 100%)",
    illustration: "talk",
  ...
  {
    gradient: "linear-gradient(160deg, #6b5fc7 0%, #9b8fe0 45%, #f7d9c9 100%)",
  ...
  {
    gradient: "linear-gradient(160deg, #7c6fd1 0%, #e2a4a0 55%, #f3d9c4 100%)",
    illustration: "ready",
...
function Illustration({ kind }) {
  if (kind === "talk") {
    return (
      <svg viewBox="0 0 220 220" fill="none" stroke="#fff" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round">
        <path d="M40 60h140a20 20 0 0 1 20 20v50a20 20 0 0 1-20 20H110l-25 25v-25H40a20 20 0 0 1-20-20V80a20 20 0 0 1 20-20Z" />
        <circle cx="80" cy="105" r="5" fill="#fff" stroke="none" />
        <circle cx="110" cy="105" r="5" fill="#fff" stroke="none" />
        <circle cx="140" cy="105" r="5" fill="#fff" stroke="none" />
      </svg>
    );
  }
  if (kind === "privacy") {
    return (
      <svg viewBox="0 0 220 220" fill="none" stroke="#fff" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round">
        <rect x="55" y="30" width="110" height="180" rx="18" />
        <line x1="90" y1="42" x2="130" y2="42" />
        <path d="M110 90a30 30 0 0 1 30 8v25c0 20-14 33-30 40-16-7-30-20-30-40V98a30 30 0 0 1 30-8Z" />
        <circle cx="110" cy="128" r="6" fill="#fff" stroke="none" />
      </svg>
    );
  }
  return (
    <svg viewBox="0 0 220 220" fill="none" stroke="#fff" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round">
      <rect x="50" y="40" width="120" height="150" rx="14" />
      <rect x="85" y="28" width="50" height="24" rx="8" fill="#fff" stroke="none" opacity="0.9" />
      <path d="M72 100l14 14 26-26" />
      <line x1="72" y1="145" x2="148" y2="145" />
      <line x1="72" y1="165" x2="130" y2="165" />
    </svg>
  );
}
    ...
    <div className="onboard-screen">
      <div className="onboard-hero" style={{ background: slide.gradient }}>
        ...
        <div className="onboard-topbar">
          ...
          <button className="skip-btn" onClick={onGetStarted} type="button">
            Skip
          </button>
        </div>
...
function ChatScreen({ messages, setMessages, userTurnCount, setUserTurnCount, showNudge, setShowNudge }) {
  ...
  function send(text) {
    ...
    setTimeout(() => {
      setTyping(false);
      setMessages((m) => [...m, { role: "agent", text: nextAgentMessage(newUserCount - 1) }]);
      if (shouldShowProviderNudge(newUserCount)) {
        setShowNudge(true);
      }
    }, AGENT_TYPING_MS);
  }
      ...
      <div className="chat-thread" ref={threadRef}>
        ...
        {messages.map((m, i) => (
          <div className={`bubble-row ${m.role}`} key={i}>
            {m.role === "agent" && <AgentMark size="sm" />}
...
function DocumentsScreen({ docs, setDocs }) {
  const [source, setSource] = useState("you");
  const fileRef = useRef(null);

  function addMockDoc(fileList) {
  ...
  return (
    <div className="screen-pad">
      ...
      <p className="subtext">
        Upload a discharge note, chart, or history. Your provider can add
        records too — everything stays on this device.
      </p>

      <div className="doc-source-toggle" style={{ marginTop: 18 }}>
        <button
          className={source === "you" ? "active" : ""}
          onClick={() => setSource("you")}
          type="button"
        >
          Uploading as you
        </button>
        <button
          className={source === "provider" ? "active" : ""}
          onClick={() => setSource("provider")}
          type="button"
        >
          Uploading as provider
        </button>
      </div>
      ...
      <div className="upload-zone" onClick={() => fileRef.current?.click()}>
        ...
        <input
          ...
          multiple
          hidden
          onChange={(e) => addMockDoc(e.target.files)}
        />
      </div>
          ...
          {docs.map((d) => (
            <div className="card doc-card" key={d.id}>
              ...
              <div className="doc-meta">
                ...
                <p className="doc-sub">
                  <span className={`source-tag ${d.source}`}>
                    {d.source === "you" ? "You" : "Provider"}
                  </span>
...
function SummaryScreen({ userTurnCount }) {
  const data = buildSummary(userTurnCount);
  const progressPct = Math.min(100, Math.round((userTurnCount / totalStages) * 100));
    ...
    <div className="screen-pad">
      ...
      <div className="section-list">
        ...
        {data.questions.length > 0 && (
          <div className="card section-card">
            <div className="section-title-row">
              <span className="sicon questions">
                <Icon name="flag" />
              </span>
...
function App() {
  ...
  // Chat state lives here so Summary tab can react to it live.
  ...
  return (
    <>
      <style>{`
@import url('https://fonts.googleapis.com/css2?family=Fraunces:ital,opsz,wght@0,9..144,500;1,9..144,500;1,9..144,600&family=Plus+Jakarta+Sans:wght@400;500;600;700;800&display=swap');
:root {
  --page-bg-1: #efeaf9;
  --page-bg-2: #f7f2e9;
...
body {
  ...
  background-image:
    ...
    radial-gradient(ellipse 700px 500px at 100% 100%, #f6e8ea, transparent 55%);
...
/* ---------- Page shell (desktop preview chrome around the phone) ---------- */
...
.chat-input-bar input:focus {
  ...
  box-shadow: 0 0 0 3px var(--lavender-tint);
}
...
.doc-source-toggle button.active {
  background: var(--lavender);
  border-color: var(--lavender);
  color: #fff;
}
...
.section-card.flags .section-list-items li {
  color: #7a2812;
}

.section-card.flags .section-list-items li::before {
  background: #a13b21;
}
...
.profile-row .rtext .rtitle {
  font-size: 0.87rem;
  font-weight: 700;
  margin: 0;
}

.profile-row .rtext .rsub {
  font-size: 0.74rem;
  color: var(--ink-faint);
  margin: 1px 0 0;
}
...
`}</style>
      ...
      {!started ? (
        ...
        <>
          ...
          {tab === "summary" && <SummaryScreen userTurnCount={userTurnCount} />}
